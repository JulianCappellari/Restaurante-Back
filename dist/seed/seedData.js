...
async function seedData() {
    try {
        ...
        await dbConfig_1.default.query(`SET FOREIGN_KEY_CHECKS = 0;`);
        ...
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS order_item_customizations;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS order_items;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS orders;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS bookings;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS dish_customizations;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS inventory;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS payment_methods;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS addresses;`);
        // Luego las tablas principales
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS menu;`); // Cambiado de 'menus' a 'menu'
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS tables;`);
        await dbConfig_1.default.query(`DROP TABLE IF EXISTS users;`);
        // Finalmente, volvemos a habilitar las verificaciones de claves foráneas
        await dbConfig_1.default.query(`SET FOREIGN_KEY_CHECKS = 1;`);
        ...
        // Usuarios
        const users = await User_1.default.bulkCreate([
            {
                firstName: "John",
                lastName: "Doe",
            ...
            },
            {
                ...
                phone: "5436548216",
                password: await bcrypt_1.default.hash("password456", 10),
                rol: "Customer",
            },
            {
                firstName: "Carlos",
                ...
                phone: "3436548213",
                password: await bcrypt_1.default.hash("password789", 10),
                rol: "Waiter",
            },
        ], { returning: true });
        // Mesas
        ...
        // Direcciones
        const addresses = await Address_1.default.bulkCreate([
            {
                street: "Calle Falsa",
                streetNumber: "123",
                city: "Córdoba",
        ...
        const menuItems = await Menu_1.default.bulkCreate([
            {
        ...
        await DishCustomization_1.default.bulkCreate([
            {
        ...
        // Inventario
        await Inventory_1.default.bulkCreate([
        ...
        const cvv1 = "123";
        const cvv2 = "456";
        const paymentMethods = await PaymentMethod_1.default.bulkCreate([
            {
                name: "Visa Gold",
                cardHolderName: "Jane Smith",
                cardNumber: await bcrypt_1.default.hash(card1, 10),
                last4: card1.slice(-4),
                expirationDate: "12/26",
                cvv: await bcrypt_1.default.hash(cvv1, 10),
                status: true,
                userId: users[1].id,
            },
            {
                ...
                cardNumber: await bcrypt_1.default.hash(card2, 10),
                last4: card2.slice(-4),
                expirationDate: "11/27",
                cvv: await bcrypt_1.default.hash(cvv2, 10),
                status: true,
                userId: users[2].id,
            },
        ], { returning: true });
        // Bookings
        await Booking_1.default.bulkCreate([
        ...
        const orders = await Order_1.default.bulkCreate([
            {
        ...
        await OrderItem_1.default.bulkCreate([
            {
